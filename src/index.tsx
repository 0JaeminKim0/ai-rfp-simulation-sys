...
app.post('/api/demo/presentation-evaluation', async (c) => {
  try {
    ...
    // 발표 평가 결과 구성
    console.log('[데모 발표] KHNP 발표 평가 데이터 생성 중...');
    const presentationEvaluation = {
      customer_id: 'demo-customer',
      presentation_title: 'KHNP 수력양수 발전 운영체계 고도화 제안',
      stt_transcript: sampleSTT,
      speech_metrics: speechMetrics,
      scores: evaluationScores,        // 100점 만점 직접 평가
      total_score: totalScore100,      // 100점 만점 총점
      overall_feedback: `전력산업과 수력양수 발전 분야의 전문성이 뛰어나게 드러나며, 체계적이고 실현가능한 운영체계 개편 방안을 제시했습니다. 
        발표 구성과 논리적 흐름이 우수하고 미래지향적 관점에서 문제를 분석했으나, 구체적인 구현 방법론과 경제성 분석이 더 보강되면 좋겠습니다. 
        전반적으로 KHNP의 미래 발전 방향을 제시한 전문적이고 설득력 있는 발표였습니다.`,
      created_at: new Date().toISOString()
    }
...
고객의 성격과 우선순위를 반영하여 실제 비즈니스 현장에서 나올 법한 현실적이고 구체적인 피드백을 작성해주세요.`
    ...
    try {
      ...
      feedbackData = {
        ...
        expected_questions: [
          '제안하신 솔루션이 경쟁사 대비 차별적 가치를 제공한다고 보장할 수 있는 근거는 무엇입니까?',
          '프로젝트 일정과 마일스톤이 현실적이라고 판단하셨는데, 지연 발생 시 어떤 리스크 관리 방안을 갖고 계신가요?',
          '유사 프로젝트 성공 사례가 있다고 하셨는데, 그 경험을 이번 프로젝트에 구체적으로 어떻게 적용할 수 있습니까?',
          '유지보수와 사후 지원을 말씀하셨는데, 예상치 못한 이슈가 발생했을 때 SLA(Service Level Agreement) 수준에서 보장할 수 있는 범위는 어디까지입니까?'
        ]
      }
    }
...
app.get('/customer-generation', (c) => {
  return c.html(`
    ...
    <body>
        ...
        <script src="https://cdn.jsdelivr.net/npm/axios@1.6.0/dist/axios.min.js"></script>
        <script src="/static/customer-generation.js?v=1.5"></script>
    </body>
    </html>
  `)
})
